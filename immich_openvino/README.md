...
⚠️ 该项目正在积极开发中。预期会有错误和变化。不要将其作为存储照片和视频的唯一方式！（来自开发者）
...
_感谢所有在我的仓库上标星的人！要标星，请点击下面的图片，然后在右上角会出现。谢谢！_
...
基于网络的文件浏览器。
该插件基于来自imagegenius的[docker映像](https://github.com/imagegenius/docker-immich)。
...
请注意，您需要安装一个单独的postgres插件才能连接数据库。您可以在我的仓库中已经安装postgres插件。
在启动之前请务必更改密码；之后它不会更改。
