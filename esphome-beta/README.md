...
这个插件允许您编写配置，并通过 Home Assistant 
将您的微控制器转变为智能家居设备 **无需编程经验**。
您需要做的就是编写 YAML 配置文件；其余的（无线更新，编译）全部由 ESPHome 处理。
...
使用 ESPHome，您只需几行 YAML 就可以直接生成自定义固件。例如，要包含一个 [DHT22][dht22]
