...
FROM $FROM_ARCH/python:3.7-alpine as docker.1ms.run/base
...
FROM docker.1ms.run/base as builderfinal

# add build utils (gcc, others)
RUN apk add build-docker.1ms.run/base

FROM docker.1ms.run/base
