...
这个插件是出色的 [rtl_433](https://github.com/merbanan/rtl_433) 项目的一个简单封装，它通过 [支持的 SDR 加密狗](https://triq.org/rtl_433/HARDWARE.html) 接收无线传感器数据，解码并以多种格式输出，包括 JSON 和 MQTT。rtl_433 理解的无线传感器主要在 433.92 MHz、868 MHz、315 MHz、345 MHz 和 915 MHz ISM 频段上发送数据。
...
默认情况下，rtl_433 会将收到的数据打印到终端 — 由您来配置它以将数据发布到 MQTT，以便 Home Assistant 可以访问，这可以通过在配置文件中添加一行来完成。
...
要使用此插件，您需要以下条件：
 ...
 2. 在一台连接了 SDR 加密狗的计算机上运行 Home Assistant OS。
...
## 安装
 ...
 1. 创建一个 rtl_433 配置文件，完成您的需求。如果您在与运行 Home Assistant OS 的计算机不同的计算机上进行此操作，可能会更好，这样您就可以自由实验并迭代，直到得到一个良好的配置。请参见下面的详细信息。
...
对于“零配置”设置，安装 [Mosquitto broker](https://github.com/home-assistant/addons/blob/master/mosquitto/DOCS.md) 插件。虽然其他代理可能有效，但未经过测试，且需要手动设置。一旦插件安装完成，启动或重新启动 rtl_433 和 rtl_433_mqtt_autodiscovery 插件以开始捕获已知的 433 MHz 协议。
...
请注意，由于配置文件中包含 bash 变量，**美元符号和其他特殊的 shell 字符需要进行转义**。例如，要在配置文件中使用字面字符串 `$GPRMC`，请使用 `\$GPRMC`。

`retain` 选项控制 MQTT 的 `retain` 标志默认是否启用。可以通过在 `output` 设置中将 `retain` 设置为 `true` 或 `false` 来逐个无线电项覆盖此设置。

在手动配置时，假设您打算将 rtl_433 数据导入 Home Assistant，在配置文件中您需要指定的绝对最小内容是 [MQTT 连接和身份验证信息](https://triq.org/rtl_433/OPERATION.html#mqtt-output):

```
output      mqtt://HOST:PORT,user=XXXX,pass=YYYYYYY
```

rtl_433 默认监听 433.92MHz，但即使这是您需要的，显式指定频率也是一个好主意，以避免混淆：
...
最后，但并非最不重要，如果您决定使用 MQTT 自动发现脚本或插件，其文档建议将 rtl_433 输出的所有数据中的单位转换为国际单位制（SI）：

```
convert     si
```

假设您只有一个 USB 加密狗连接且 rtl_433 能够自动找到它，我们得到一个最小的 rtl_433 配置文件如下所示：
