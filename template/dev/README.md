...
此附加组件允许您编写配置，并直接通过 Home Assistant 将您的微控制器转换为智能家居设备**无需编程经验**。
您所需做的就是编写 YAML 配置文件；其余的（空中更新、编译）都是由 ESPHome 处理。
...
使用 ESPHome，您可以从几行 YAML 直接到定制固件。例如，要包含一个 [DHT22][dht22] 温湿度传感器，您只需在您的配置文件中包含 8 行 YAML：
