...
此附加组件允许您编写配置并通过 Home Assistant 将微控制器直接转变为智能家居设备，**无需编程经验**。
您只需要编写 YAML 配置文件；其余部分（空中更新、编译）均由 ESPHome 处理。
...
使用 ESPHome，您可以从几行 YAML 直接生成定制固件。例如，要包括一个 [DHT22][dht22] 温度和湿度传感器，您只需要在配置文件中包含 8 行 YAML：
