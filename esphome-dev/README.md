...
此附加组件允许您通过Home Assistant **无需编程经验**，直接编写配置并将微控制器转化为智能家居设备。您只需编写YAML配置文件；其余的（空中更新、编译）都由ESPHome处理。
...
使用ESPHome，您可以从几行YAML直达定制的固件。例如，要包括一个[DHT22][dht22]温湿度传感器，您只需在配置文件中包含8行YAML：
