...
这个附加组件允许您编写配置，将您的微控制器直接转变为智能家居设备，通过 Home Assistant **无需编程经验**。您只需编写 YAML 配置文件；其余的（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，您可以从几行 YAML 直接生成自定义固件。比如，要包含一个 [DHT22][dht22] 温湿度传感器，您只需在配置文件中加入 8 行 YAML：
