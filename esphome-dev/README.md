...
该插件允许您通过Home Assistant直接编写配置，将微控制器转变为智能家居设备，**无需编程经验**。您只需编写YAML配置文件；其余的（空中更新、编译）都由ESPHome处理。
...
使用ESPHome，您可以从几行YAML直接生成定制的固件。例如，要包括一个[DHT22][dht22]温度和湿度传感器，您只需在配置文件中包含8行YAML：
