...
这个附加组件允许你通过 Home Assistant 编写配置，把你的微控制器变成智能家居设备，**无需编程经验**。
你需要做的就是编写 YAML 配置文件；其余的（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，你可以从几行 YAML 代码直接生成定制的固件。例如，要包括一个 [DHT22][dht22] 温湿度传感器，你只需在配置文件中包含 8 行 YAML 代码：
