...
此插件允许你直接通过 Home Assistant 编写配置，将你的微控制器变成智能家居设备**无需编程经验**。
你需要做的只是编写 YAML 配置文件，其余的（如空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，你可以通过几行 YAML 代码直接生成定制的固件。例如，要包含一个 [DHT22][dht22] 温湿度传感器，你只需在配置文件中加入 8 行 YAML 代码：
