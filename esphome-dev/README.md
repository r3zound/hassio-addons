...
这个插件允许你编写配置，并将你的微控制器
直接通过 Home Assistant 转变为智能家居设备 **无需编程经验**。
你所需要做的只是编写 YAML 配置文件；其余部分（空中更新、编译）都是由 ESPHome 处理的。
...
使用 ESPHome，你可以从几行 YAML 直接生成定制固件。例如，要包括一个 [DHT22][dht22] 温湿度传感器，你只需在你的配置文件中添加 8 行 YAML：
