...
这个附加组件允许你通过 Home Assistant 编写配置
并直接将你的微控制器变成智能家居设备 **无需编程经验**。
你所需要做的就是编写 YAML 配置文件；其他（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，你可以从几行 YAML 直接生成定制的
固件。例如，要包含一个 [DHT22][dht22]
温湿度传感器，你只需要在你的配置文件中包含 8 行 YAML：
