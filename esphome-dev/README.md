...
此附加程序允许您编写配置文件，并通过 Home Assistant 将微控制器直接变成智能家居设备，**无需编程经验**。您只需编写 YAML 配置文件；其余（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，您可以从几行 YAML 直接生成自定义固件。例如，要包含一个 [DHT22][dht22] 温度和湿度传感器，您只需在配置文件中包含 8 行 YAML：
