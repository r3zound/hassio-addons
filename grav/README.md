...
我在业余时间维护这个以及其他 Home Assistant 插件：跟踪上游变更、Home Assistant 变更以及在实际硬件上的测试都需要花费大量时间（以及一些金钱）。我经常使用大约 5-10 个我超过 110 个插件中的几个，因此我安装了测试机器（并购买了一些我自己不使用的测试服务，例如 VPN），以用于调试和改进插件。
...
[Grav](https://getgrav.org) 是一个免费的自托管内容管理系统（CMS），使用 PHP 编程语言编写，基于 Symfony 网络应用程序框架。它使用平面文件数据库作为后端和前端。
...
使用插件的 `env_vars` 选项来传递额外的环境变量（名称为大写或小写）。有关详细信息，请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

WebUI 可在 <http://homeassistant:9191> 找到。
